...
const MemoryPalaceDisplay: React.FC<MemoryPalaceDisplayProps> = ({ 
  ...
  const handleRegenerate = async () => {
    ...
    await onRegenerateImages(editPrompt, selectedImageIndex, activeGenerationIndex);
    setIsEditing(false);
    setEditPrompt('');
  };
            ...
            <div className="relative rounded-xl overflow-hidden shadow-2xl border border-gray-200">
              ...
              {!isEditing && (
                  <button
                    ...
                    className="absolute top-4 right-4 bg-white/80 backdrop-blur-sm p-3 rounded-full text-gray-800 hover:bg-white hover:scale-110 transition-all duration-200 shadow-md"
                ...
                {currentImageSet.map((url, index) => (
                  <div key={`${activeGenerationIndex}-${index}`} className="relative group">
                    ...
                    <button
                      ...
                      className="absolute top-1.5 right-1.5 bg-white/70 backdrop-blur-sm p-1.5 rounded-full text-gray-700 hover:bg-white hover:text-indigo-600 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all duration-200"
